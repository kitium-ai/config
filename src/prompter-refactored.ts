 ...
 */
export class ConfigPrompterRefactored {
  ...
  async prompt(granularMode: boolean = false): Promise<SetupChoices> {
    ...
    const selectionMode = granularMode
      ? 'granular'
      : await this.promptSelectionMode();
  ...
  private async promptSelectionMode(): Promise<'group' | 'granular'> {
    const response = await enquirer.prompt<{ mode: 'group' | 'granular' }>({
      type: 'select',
      name: 'mode',
  ...
  private async promptGranularFileSelection(packageType: PackageType): Promise<ConfigFile[]> {
    ...
    for (const [group, configs] of configsByGroup) {
      ...
      const response = await enquirer.prompt<{ files: ConfigFile[] }>({
        type: 'multiselect',
        name: 'files',
  ...
  ): Array<{ name: TestFramework; message: string }> {
  ...
  private getRecommendedTestRunnerIndex(
    detectedFrameworks: TestFramework[],
    choices: Array<{ name: TestFramework; message: string }>
  ): number {
